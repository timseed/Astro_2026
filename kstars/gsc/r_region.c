...
*/
	plate = (char *)malloc((nrec+1)*5);
	table = (char *)malloc((nrec+1)*(lrec+1));
	
	for(i=0;i<nrec;i++) {
		read(0,fits,lrec);
		*(fits+lrec) = '\0';
                strncpy(plate+i*5,fits+40,4);
                *(plate+i*5+4) = '\0';
		strncpy(table+(i*(lrec+1)),fits,lrec+1);
		*(fits+14) = '\0';
		ra = atof(fits+5);
                if(i>0 && ra>(amax+60.)) ra -= 360.;
                if(i>0 && ra<(amin-60.)) ra += 360.;
                if(ra < amin) amin = ra;
                if(ra > amax) amax = ra;
	}
...
/* ---- sort by dec and find dec-min dec-max ----------------------- */
	...
	qsort(table,nrec,lrec+1,compar1);
        strncpy(str,table+14,9); str[9] = '\0';
        dmin = atof(str);
        strncpy(str,table+(nrec-1)*(lrec+1)+14,9); str[9] = '\0';
        dmax = atof(str);
